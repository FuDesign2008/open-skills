...
- 在 spec Requirement 1 显式确立「单轮单问、多轮逐个（允许多个未知）」+「不得误解为全程只能问一个」反向约束
- 各 skill 的纯歧义标题/首句统一为「单轮单问、多轮逐个」式表述
...
### D1：统一用语「单轮单问、多轮逐个」/ "one question per turn, across multiple turns"
...
Requirement 1 是单轮-vs-全程语义的核心。Requirement 4（高频强化）、5（skill 落地）规定的是「强化方式」「落地形态」，不规定具体字眼；skill 用「单轮单问、多轮逐个」仍满足 4/5。故 delta 最小化，只改 Requirement 1，并新增 1 个 Scenario「不得误解为全程只能问一个问题」。

### D3：各 skill 改动分级（按当前歧义程度）

| skill | 位置 | 当前 | 动作 |
|-------|------|------|------|
| solve-workflow | 127 标题 + 129 首句 | 「一次只问一个」（纯歧义源） | **必改**：标题→「单轮单问、多轮逐个」；首句联动 |
...
- **think-big 英文与中文不统一** → 英文用对仗表述 "one question per turn, across multiple turns"，语义等价
