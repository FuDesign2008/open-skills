...
定义 `runtime-verification-discipline` 的纪律契约：所有 PDCA 工作流的验证环节，默认由 AI 在「模拟或真实」环境中亲自执行验证（而非把验证步骤列成清单交还人）。真实环境优先、模拟环境兜底，验证能力由可插拔提供者解析，仅在真硬边界才交还人且必须给出理由。目标是让 AI 承接验证劳动、减少人工干预，同时不以降低验证真实性为代价。

本 skill 是规则层（纪律），不提供具体环境驱动能力；通道选择与驱动 recipe 委派给各能力 skill（如 `browser-debug-toolkit`）。
