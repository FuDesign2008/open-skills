...
- **THEN** 其正文只保留决策顺序规则与核查项名称内联（约两行），完整核查方法指向 `code-design-review` Layer B，不逐字复制
