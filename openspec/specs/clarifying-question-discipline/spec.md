...
此约束规范的是**单轮交互**：一条消息只含 1 个问题（**一次一问**）。当存在多个未知时，AI MUST 通过多次一问一答、分多轮把问题问清（**多轮问清**——需要几个就分几轮问），且每一轮的问题 MUST 基于上一轮回答来精化/确定（渐进收敛，而非机械按预定清单提问）；MUST NOT 把「一次只问一个」误解为「全程只能问一个问题」而自我设限、省略后续必要的追问、带着信息缺口继续（过度收敛）。防「多问题同抛」与防「该追问不追问」是该纪律对称的两面。
