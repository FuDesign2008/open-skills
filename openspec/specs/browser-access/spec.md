# browser-access Specification

## Purpose
open-skills 内部公共浏览器操控底座的共享契约：通过 CDP Proxy 直连用户日常浏览器（天然携带登录态）+ HTTP API 操控动态页面/交互/媒体/视频 + 站点经验积累 + 子 Agent 并行分治。被 `effective-web-research` 与 `browser-debug-toolkit` 通过 frontmatter `dependencies` 强依赖，也可被用户直接调用。
