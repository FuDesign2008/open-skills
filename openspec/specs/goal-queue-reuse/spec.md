# goal-queue-reuse Specification

## Purpose
Cross-run solution reuse for the goal-driven-queue backlog, owned by the user-invocable `goal-driven-queue` skill: marking a card's frozen approach reusable with its problem signature, the verification that proved it and the directions already falsified, seeding a new equivalent card's intake from such a record so the same problem class is not re-interviewed, and delegating promotion into a shared carrier to `learn-and-improve`'s carrier decision tree. The queue's own records stay inside `.goal-driven/`.
## Requirements
