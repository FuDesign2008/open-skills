# perf-evidence-discipline Specification

## Purpose
Evidence-validity hard gate for performance work: nine disciplines (measurement artifacts / caliber pollution / device-profile mismatch) that a metric must clear before it may drive an optimization decision; mounted by `perf-optimize-workflow` and other hosts via frontmatter `dependencies`, never the reverse.
