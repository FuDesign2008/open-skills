# explain-simply Specification

## Purpose

Dual-track technical concept explanation owned by the `explain-simply` skill: answer concept/mechanism questions with a rational track (accurate mental model grounded in authentic evidence) plus an intuitive track (fidelity-preserving analogy and a memory anchor), enriched with visual aids when they clarify. Distilled from real Q&A material (`docs/concept-explanation-dual-track.md`).
