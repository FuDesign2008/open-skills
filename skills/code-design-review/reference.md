...
1. [Layer A — Architecture-level quality attributes](#layer-a--architecture-level-quality-attributes)
2. [Layer B — Code-level design metrics](#layer-b--code-level-design-metrics)
...
These evaluate the solution's impact on system-level quality — the layer that carries architectural design and long-term maintainability weight. Scale by path: quick path (small isolated change) may do a fast pass; full path (cross-module, architecture-affecting) must assess all.
...
- **How to apply**: Trace the blast radius of the proposed change. If the change forces edits in modules that have no logical relationship to the change, modularity is poor. (Overlaps with Shotgun Surgery in Layer B — Layer A looks at the system level; Layer B at the code level.)
...
- **Architecture-boundary verification** (solutions crossing process or layer boundaries, e.g. host/main process ↔ service subprocess, thin shell ↔ product layer):
  1. **Runtime initialization location** — Identify which process/layer actually initializes the called capability (FFI/native handles, databases, caches, service singletons are often initialized in a service subprocess, not the caller). If the caller's process has no runtime for it, the design is broken regardless of how small the diff is.
  2. **Boundary legality** — Check whether a cross-layer static or dynamic import would pull the callee's dependency tree into the caller's bundle graph, triggering dormant circular dependencies. Note: bundlers statically pre-scan `require`/`import` targets into the bundle, so converting a static import to a dynamic `require` at the call site does **not** change module evaluation order or avoid the cycle — treat that argument as no mitigation.
  3. **Ownership classification** — Classify the capability: system capability (clipboard, file dialogs, window management — belongs in the shell/host layer) vs data/product capability (search, storage, domain services — belongs in the service/product layer). A solution that reaches across this classification against the calling layer's positioning is an ownership mismatch; prefer routing the call through the layer that owns the capability.
- **Blocking (boundary-crossing)**: called capability has no runtime in the caller's process/layer, or a cross-layer import pulls an unrelated dependency tree into the caller — unless explicitly accepted as Prudent-Deliberate debt with a repayment plan. Short-term cost advantages (reuse / single-repo change / no cross-team coordination) do not downgrade it.
...
- **Myers coupling types** (ordered from acceptable to unacceptable):
  ...
  - **Content coupling** — one module directly reads/writes another's internal data. Unacceptable; no module should reach into another's internals.
...
|---|-----------|--------|------|--------|
| 1 | Clean Architecture (testability) | Robert C. Martin | 2017 | *Clean Architecture* |
| 2–4 | ISO/IEC 25010 quality model | ISO/IEC | 2011 | ISO standard |
| 5 | Stable Dependencies Principle | Robert C. Martin | — | butunclebob.com |
| 6 | No Silver Bullet (accidental complexity) | Fred Brooks | 1986 | IEEE Computer / UNC TR |
| 7a | Coupling classification | Glenford Myers | 1975 | *Reliable Software Through Composite Design* |
| 7b | Connascence | Meilir Page-Jones | 1996 | *What Every Programmer Should Know About OO Design* |
| 8 | Cohesion gradient | Yourdon & Constantine | 1979 | *Structured Design* |
| 9a | Shotgun Surgery (code smell) | Martin Fowler | 1999 | *Refactoring* |
| 9b | Open-Closed Principle | Bertrand Meyer | 1988 | *Object-Oriented Software Construction* |
| 10 | Technical Debt Quadrant | Martin Fowler | 2009 | martinfowler.com |
| 11a | Cyclomatic Complexity | Thomas McCabe | 1976 | IEEE TSE |
| 11b | Cognitive Complexity | SonarSource | 2016 | SonarSource white paper |
| 12 | Law of Demeter | Lieberherr & Holland | 1989 | OOPSLA |
