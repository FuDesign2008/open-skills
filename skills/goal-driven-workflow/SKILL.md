...
- `ai-counterpart-discipline` — counterpart checkpoints (`Stage-exit policy: counterpart`): absent-mode intake Q&A, bounded contract approval, stage 5 report check; abort only when a card carries that policy and the skill is missing
...
> 🎭 **Stage-exit policy**: record `manual-pause | counterpart | auto-escape` in Template 1 (queue children inherit it from the card's interaction-budget ticket; it overrides trigger-word mode propagation). `counterpart` = `ai-counterpart-discipline` occupies the human seat at the enumerated checkpoints (absent-mode intake Q&A, contract approval as bounded pre-authorization, stage 5 report check) under its charter; `manual-pause` = manual mode pauses; `auto-escape` = auto with named escapes (legacy default).
