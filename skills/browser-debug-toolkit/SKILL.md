...
This skill is discovered by the environment capability exploration mechanism of `solve-workflow`, `debug-workflow`, and similar workflow skills. It is also delegated to by `hybrid-fullstack-debugging` for runtime evidence in hybrid app (WebView/WKWebView/Electron + H5) debugging scenarios:
