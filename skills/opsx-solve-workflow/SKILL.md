...
**⚠️ 一次只问一个关键问题（硬纪律，无条件适用）**：信息不足时，每次只问 **1 个最关键的问题**（优先级：目的 → 约束 → 成功标准），得到回答后再问下一个——**不得一次抛多个**。提问用「单问题 + 多选项」格式（平台无关，见 solve-workflow「主动提问」提问格式）；Claude Code 上可用 AskUserQuestion 工具将其工具化，Cursor / OpenCode 等无此工具时直接用 prose 格式即可。无论是否检测到 brainstorming skill，此纪律都必须遵守（brainstorming 的提问纪律是通用做法，不是触发条件）。
