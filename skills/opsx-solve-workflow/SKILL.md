...
1. **环境能力探索**：使用当前平台的 skill 发现能力，检查可用的 skill、agent、插件列表。按以下能力类型关键词匹配可用 skill/agent 的 name 和 description：
   ...
   | 🌐 Web 调研 | research, web, look up, investigate, web-research | 阶段 1.2（步骤 2.5/3.5/5.5） | 外部 web 调研纪律：Step 0 分流 + 4 口诀 + 严格模式 |
...
| 5. 日志分析 | 分析实际调用链和数据流，给出根因结论，更新 `proposal.md` 的 Why / Impact 小节 |
...
2. **结果处理**：找到 → 更新根因假设，回到步骤 5；未找到 → 输出「已排除假设清单」，**暂停等用户决定**（继续打点 / 外部支持 / 带假设进入阶段 2 探索方案）
