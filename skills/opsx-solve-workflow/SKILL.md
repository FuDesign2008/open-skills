...
**⚠️ 一次一问、多轮问清（硬纪律，无条件适用）**：信息不足时，**每轮只问 1 个最关键的问题**（优先级：目的 → 约束 → 成功标准），**得到回答后基于该回答提出下一个、多个未知分多轮问清**。即：一条消息不堆叠多个（**不得一次抛多个**）；该问几个分几轮问清，**且每轮基于上一轮回答精化**（渐进收敛），**不得误解为「全程只能问一个」**而带着信息缺口继续。提问用「单问题 + 多选项」格式（见 solve-workflow「主动提问」提问格式）；优先用你所在 Agent 的原生结构化提问能力，无则用 prose。无论是否检测到 brainstorming skill，此纪律都必须遵守（brainstorming 的提问纪律是通用做法，不是触发条件）。
