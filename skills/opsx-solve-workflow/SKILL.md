...
**⚠️ 单轮单问、多轮逐个（硬纪律，无条件适用）**：信息不足时，每次只问 **1 个最关键的问题**（优先级：目的 → 约束 → 成功标准），得到回答后再问下一个——**不得一次抛多个**。提问用「单问题 + 多选项」格式（见 solve-workflow「主动提问」提问格式）；优先用你所在 Agent 的原生结构化提问能力，无则用 prose。无论是否检测到 brainstorming skill，此纪律都必须遵守（brainstorming 的提问纪律是通用做法，不是触发条件）。
...
- 1.1 一次抛出多个疑问点让用户回答（违反「单轮单问、多轮逐个」硬纪律）——每次只问 1 个最关键的，得到回答后再问下一个
