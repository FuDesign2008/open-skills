...
3.6 **上游依赖修复评估**（可选，根因明确为上游依赖 bug 时触发——与 3.5 配对的「乐观分支」）
   - **触发条件**（满足任一）：根因明确为上游依赖 bug；或步骤 2.5 找到上游已修复版本。
   - **核心原则**：遇到 bug 优先评估「升级依赖」而非写 workaround。加载 `upstream-dependency-debug` skill，按其方法论执行——4 步决策顺序 + 反模式 + 通用升级工程纪律 + 结果表。
   - **4 步决策顺序**（排查根因来源）：
     1. 先判断 bug 更可能来自**上游依赖**还是**本仓库代码**（症状是否与平台/框架/库的特定行为强相关、是否仅在特定环境复现）
     2. 若疑似上游：查依赖的 Changelog / Release Notes / Issues，确认是否已有修复版本
     3. **优先升级依赖**（patch/minor，遵循 semver 低风险），而非立即写本地 workaround
     4. 只有当升级不可行（breaking change、版本锁定、上游未修复）时，才考虑 workaround —— 且 workaround 需注明是临时方案及触发条件
   - **反模式（避免）**：未查上游就直接堆 workaround，导致 magic number 时序、UX 降级、平台强制行为 CSS 无解等问题累积成技术债。
   - **通用升级工程纪律**（升级决策确定后，确保安全执行）：
     - **包管理器一致性**：用项目实际包管理器（`git ls-files package-lock.json yarn.lock pnpm-lock.yaml` 判定 tracked 的是哪个），不混用；注意系统级 `packageManager` 字段可能与项目实际不一致
     - **升级后验证链**：typecheck + build + 全量单测 + 真机/目标环境验证
     - **semver 风险评估**：patch/minor 低风险（库遵循 semver 且无 breaking change）；major 需评估 breaking change
     - **dedup 检查**：升级后 `npm ls <pkg>`（或对应工具）确认单版本，避免多版本共存导致类型不兼容
   - **结果处理**：

   | 结论 | 处理方式 |
   |------|---------|
   | ✅ 上游已修复且升级低风险 | 升级依赖作为首选方案，继续步骤 4（影响范围评估），后续进入阶段 2 方案探索时升级方案作为推荐方案 |
   | ⚠️ 升级有风险（major/breaking） | 升级方案与 workaround 方案并列，继续步骤 4，后续进入阶段 2 对比 |
   | ❌ 上游未修复或升级不可行 | 回到 workaround，但注明临时性 + 触发条件，继续步骤 4 |
