...
> **Retry-scenario preferred strategy**: Static analysis has already been performed once; repeating it will likely hit the same boundary. Prefer instrumentation debugging to re-anchor the root cause with runtime facts, rather than retracing the previous round's analysis path.
