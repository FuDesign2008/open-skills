...
扫描 `.claude/skills/` 目录，检查是否存在以下必需的原生 OPSX skills（名称含 `openspec-` 或 `opsx-` 前缀）：

| 所需 skill | 对应阶段 | 用途 |
|-----------|---------|------|
| `openspec-propose` 或 `opsx-propose` | 阶段 1-2 | 生成 proposal + delta specs |
| `openspec-apply` 或 `opsx-apply` | 阶段 5 | 按 tasks 执行实现 |
| `openspec-archive` 或 `opsx-archive` | 阶段 7 | 归档 change |

`openspec-verify` / `opsx-verify` 为可选 skill，存在时在阶段 6 使用。
...
3. 准备创建方式，但不得在阶段 1.1 用户确认前创建目录：通过 `openspec-propose` skill 创建 change（读取其 SKILL.md，按其指令执行）。
...
| 6. 检查验证 | 测试、校验、对照 artifacts | 验证结论（通过 `openspec-verify` skill 或 `openspec validate`） | 禁止 |
| 7. 回顾归档 | 沉淀结果或进入下一轮 | `openspec/specs/` 更新 + `openspec/changes/archive/` 迁移（通过 `openspec-archive` skill） | 仅限归档/文档 |
...
1. 通过 `openspec-propose` skill 创建或选择 change（读取其 SKILL.md，按其指令执行）。
2. 记录本轮路径选择（完整 / 增量 / 精简）。
3. 再进入 1.2 技术分析。
...
🔌 **OPSX Skills 集成**：技术分析完成后，通过 `openspec-propose` skill 创建或更新 `proposal.md` 的 Why / Impact 草稿（先读取其 SKILL.md，再按其指令执行）。本 skill 不直接手写 proposal.md 内容。
...
🔌 **OPSX Skills 集成**：方案选定后，通过 `openspec-propose` skill 更新 `proposal.md` 的 What Changes / Capabilities 小节，并创建或更新 delta specs（先读取其 SKILL.md，再按其指令执行）。本 skill 不直接手写 proposal.md 或 delta spec 内容。

Delta spec 规范（由 `openspec-propose` skill 负责落实）：
...
🔌 **OPSX Skills 集成**：调用 `openspec-apply` skill 执行任务（先读取其 SKILL.md，再按其指令逐项完成 tasks）。`openspec-apply` skill 内部会通过 CLI 查询 change 状态和获取执行指令；本 skill 不直接调用 CLI 推进执行。
...
1. **OpenSpec 校验**：
   - 若检测到 `openspec-verify` skill → 读取其 SKILL.md，委托执行验证。
...
🔌 **OPSX Skills 集成**：调用 `openspec-archive` skill 执行归档（先读取其 SKILL.md，再按其指令执行）。

若 `openspec-archive` skill 执行失败，**不得**手动操作 `openspec/` 目录；应停止并提示用户检查 openspec 安装状态。
...
| 检测到原生 OPSX skills 却直接调 CLI 或手写 artifacts | 绕过 schema 模板和 context injection，artifacts 不符规范 | 委托原生 skill（先读 SKILL.md）；CLI 仅允许作为工具命令（`openspec validate` 等） |
