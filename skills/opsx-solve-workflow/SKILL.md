...
>
> ⚠️ **v2 补充（合并动作触发锚点）**：即使用户跳过分支收尾决策直接下达合并指令（「merge MR」「合并 MR」「准备合并」），或 AI 准备调用 `glab mr merge` / `gh pr merge`，**也必须先运行门控**——门控触发锚点是「合并动作」本身，不依赖是否走过分支收尾决策。完整规则见 [reference.md](reference.md)「触发条件（v2）」。

#### 合并前覆盖率门控（强制，门控规范 v2）

> 触发条件（v2）：当 AI 即将执行任何合并动作时**必须**先运行本门控，不依赖是否走过分支收尾决策。判定信号：分支收尾决策选定「合并」/ 用户直接下达合并指令 / AI 准备调用合并命令 / 用户消息含「merge」「合并」等词且上下文为合并意图。
> **执行门控前必须先读** [reference.md](reference.md)「合并前覆盖率门控（强制）规范」获取完整步骤（v2：触发条件 / 前置检测收紧 / 判定矩阵 / 隐式漏跑留痕 / 合并前检查清单 / 模式生命周期），**不得仅凭下方摘要执行**。

> ⚠️ **合并命令前置检查（强制）**：调用 `glab mr merge` / `gh pr merge` 前，AI 必须自问「是否已运行合并前覆盖率门控？」。若未运行，**先跑门控再合并**，不得直接执行合并命令。

**判定矩阵概要**（安全网，完整矩阵见 reference.md）：报告生成+达标→继续合并；不达标→暂停；脚本崩溃/无报告/退出码1→**视为门控未通过**（不得误判为通过）；无测试代码/0%通过→暂停；**AI 即将合并但未运行门控（隐式漏跑）→视为门控未通过，暂停合并补跑，已合并则如实报告漏跑并留痕**。
**本步骤独立 Bash 权限**：运行 test-coverage-analyzer 脚本，不改变阶段 8「仅限归档/文档」的工具约束本质（门控是合并子步骤而非归档动作）。
**留痕位置**（显式跳过 / 隐式漏跑 / 环境缺漏）：PR 描述和 `design.md` 的 Verification Notes。
...
| 用户直接说「merge MR / 合并 MR」但 AI 跳过门控直接合并（v2 新增场景） | 门控被绕过，未验证代码进入主分支（2026-07-22 MR !450 事件） | 门控触发锚点是「合并动作」本身，不依赖分支收尾决策；用户直接下达合并指令时也必须先跑门控 |
| AI 即将合并但未运行门控（隐式漏跑）却继续合并或合并后未留痕（v2 新增） | 隐式漏跑无法被显式跳过留痕机制捕获，门控形同虚设 | 隐式漏跑视为门控未通过；合并前发现须暂停补跑；合并后发现无法回退，必须如实报告并写入留痕 |
| 未装 test-coverage-analyzer 的项目合并时静默跳过且不留痕（v2 收紧） | 无声漏跑无法追溯 | 前置检测未发现 skill 时必须输出提示并留痕，由用户决定是否继续，不得静默放行 |
