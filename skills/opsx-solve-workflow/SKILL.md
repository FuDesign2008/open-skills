...
- `learn-and-improve`（阶段 7 复盘改进与经验沉淀）
...
- `ensure-tests`（阶段 5 测试确保：有测试基建时补全并运行；无基建经用户确认后搭建）
...
**工具限制**：✅ Read/Grep；✅ WebSearch（步骤 1.5 调研路由与外部调研、步骤 3.6 上游依赖修复评估、打点失效逃生出口专用）；❌ Edit/Write
