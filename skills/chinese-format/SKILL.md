...
- **技术术语**：JavaScript、TypeScript、React 等（技术术语与中文标点之间无需空格，直接相邻；**在输出中必须原样保留，不得转换或省略**）
- **代码**：所有代码块、代码片段（在输出中必须原样保留）
- **命令**：git commit、npm install 等（在输出中必须原样保留）
- **URL/文件路径**：网址和路径（在输出中必须原样保留）
- **数据格式**：版本号（8.2.10）、百分比（74%）、数字（3.14）（在输出中必须原样保留）
