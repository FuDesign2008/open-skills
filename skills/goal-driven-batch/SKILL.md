...
version: "0.12.0"
user-invocable: true
description: "Queue orchestrator over child engines (goal-driven / solve / opsx-solve / jira-fix / opsx-jira-fix). Triggers when the user says「goal 批量」「跑队列」「任务队列」「无人值守队列」「把需求加入队列」「空闲时段跑任务」/ goal-driven-batch, goal batch, run queue, unattended task queue. Persistent backlog, intake pre-approval, serial consumption, caps, acceptance package. Jira multi-issue lists enqueue here (also via jira-fix-batch / opsx-jira-fix-batch shells). Use to enqueue now and run later, or to start/inspect/review a queue. Do NOT use for a single ad-hoc long run (goal-driven-workflow), a single Jira issue (jira-fix-workflow / opsx-jira-fix-workflow), or setting up cron/schedulers themselves."
dependencies:
