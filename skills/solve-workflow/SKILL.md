...
| 2 分析问题 | ✅ Read/Grep；✅ WebSearch（0.5 调研路由/3.6/5.5 专用）；⚠️ 受限 Edit/Write（分析辅助性临时改动，须登记+回滚，见阶段2正文） | 继续进入阶段3 | 存在性结论／根因／影响范围／临时改动回滚验证 |
...
| 🌐 Web 调研 | 阶段2（0.5 调研路由 / 打点逃生出口） | 经 `known-issue-research` 统一委托 `effective-web-research` |
...
3.5 **Scope 拆解**（若适用）- 若问题涉及多个独立子系统（如「聊天 + 文件存储 + 计费」），先协助拆解：独立模块、依赖关系、建议处理顺序，再对首个子问题进行分析。**本步骤仍在本阶段内，禁止主动探索代码；用户已引用的内容可读取**，拆解主要依据用户描述。
4. **等待用户确认**
...
**工具限制**：✅ Read/Grep/SemanticSearch；✅ WebSearch（步骤 0.5 调研路由与快搜、步骤 3.6 上游依赖修复评估、打点失效逃生出口 5.5 专用）；✅ Edit/Write 限上述分析辅助改动；✅ Bash 运行只读验证类命令——运行被调试应用/复现步骤仍需用户确认（见「打点调试」小节）
...
**阶段 2 分析问题**：先报告存在性验证结论 → 0.5 调研路由结论（🟢/🔵/🟣）→ 再按路由侧重依次输出现象、定位、根因；0.5 的快搜/通病评估结论按 `known-issue-research/reference.md` 模板输出；若触发步骤 3.6，输出上游依赖修复评估结论；继续输出影响范围。**若本阶段做过临时改动，最后输出「临时改动清单 + 回滚验证」（见「临时改动权限与回滚门控」）。**
