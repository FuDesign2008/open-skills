...
> 启动时（进入阶段 1 之前）必须执行本检查，核对 frontmatter `dependencies` 声明的全部 13 个强依赖 skill。
...
**阶段 2 分析问题**：先报告存在性验证结论 → 调研路由结论（🟢/🔵/🟣）→ 再按路由侧重依次输出现象、定位、根因；调研路由的快搜/通病评估结论按 `known-issue-research/reference.md` 模板输出；若触发「上游依赖修复评估」步骤，输出其评估结论；继续输出影响范围。**若本阶段做过临时改动，最后输出「临时改动清单 + 回滚验证」（见 `analysis-core` §1）。**
