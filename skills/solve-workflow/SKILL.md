...
### ⚠️ 主动提问（硬纪律：单轮单问、多轮逐个）

> **单轮只问一个最关键的问题、多轮逐个推进**——这是硬纪律，不靠自觉。信息不足时，挑出当前**最关键**的 1 个未知（优先级：目的 → 约束 → 成功标准），用「单问题 + 多选项」的结构化提问（见下方提问格式）——优先用你所在 Agent 的原生结构化提问能力，无则用 prose，得到回答后再问下一个。
