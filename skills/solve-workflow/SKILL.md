...
| 8 回顾总结 | ❌ Edit/Write（除非用户确认写入或进入下一轮） | 结束 / 或循环回阶段3/4 | 改进建议＋沉淀载体 |
...
精简路径下，阶段 3（探索方案）可只输出 1 个方案 + 风险说明，阶段 5（制定计划）可合并到方案描述中。但阶段 7（检查验证）和阶段 8（回顾总结）不可跳过。
...
> 原则：只输出检查结果，不输出改进建议；改进建议在阶段 8（回顾总结）
