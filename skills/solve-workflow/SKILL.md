...
| 1.2 技术分析 | ✅ Read/Grep；✅ WebSearch（步骤2.5/3.5/3.6/5.5专用）；❌ Edit/Write | 继续进入阶段2 | 存在性结论／根因／影响范围 |
...
> 「一次一问、多轮问清」是硬纪律，不靠自觉。信息不足时，挑出当前**最关键**的 1 个未知（优先级：目的 → 约束 → 成功标准），用「单问题 + 多选项」结构化提问（见下方格式）——优先用你所在 Agent 的原生结构化提问能力，无则用 prose；**得到回答后，基于该回答提出下一个**，如此多轮问清，直到信息充分。

**两个约束，缺一不可**：
- **一次一问**——一条消息只含 1 个问题。**禁止**一次抛多个 / 一次列多个疑问点让用户逐个回答（加重认知负担、降低答复质量）。
- **多轮问清**——有多个未知就分多轮问清，**且每轮问题基于上一轮回答精化**（渐进收敛，而非机械按清单问）。**不得把「一次只问一个」误解为「全程只能问一个」**而自我设限、带着信息缺口继续——该问几个就分几轮问。

**为什么**：多问题同抛 → 用户负担重、易漏答、AI 难聚焦；一次一问、逐轮基于回答精化 → 渐进收敛。防「多问」与防「该追问不追问」是对称的两面。

各阶段进行中，若信息不足以保证输出质量，按此硬纪律**每轮只提 1 个最关键的问题**；得到回答后，基于回答决定下一步——信息已充分则继续推进，否则追问下一个未知。
...
| 🌐 Web 调研 | research, web, look up, investigate, web-research, effective-web-research | 阶段1.2（步骤 2.5/3.5/3.6/5.5） | 外部 web 调研纪律：Step 0 分流 + 4 口诀 + 严格模式（见 `effective-web-research` skill） |
...
3. **疑问点列出** - 列出需要进一步确认的地方；**若向用户提问，一次只问 1 个最关键的**（见下方「主动提问」硬纪律），得到回答后再问下一个
...
0.5 **调研路由判断**（存在性验证通过后立即执行，决定后续步骤侧重）
   ...
   判断这个**已确认存在**的问题，根因更可能落在哪类——这决定步骤 2.5/3/3.5/3.6 的执行侧重（不替换它们，只调整优先级）：

   | 路由 | 判定信号 | 后续侧重 |
   |------|---------|---------|
   | 🟢 **内部为主** | 问题讲「我们的代码/逻辑/规范」；根因疑似本仓库实现 | 步骤 1-2 代码定位为主；2.5 可选兜底；步骤 3 根因分析为核心 |
   | 🔵 **外部为主** | 具名第三方库/框架/API + 版本；或问「怎么用/为什么/有没有已知问题」；或命中下方 2.5 触发条件（平台静默失败 / 宿主嵌套 / 代码层无嫌疑） | **2.5 升为首要动作**（先 web 搜已知案例）；搜到根因：若为上游已修复跳步骤 3.6，否则跳步骤 4；搜不到回步骤 3 |
...
2.5 **已知问题快搜**（路由判定为 🔵外部/🟣hybrid 时为**首要执行**；🟢内部时为**可选兜底**——保留下方触发条件作为内部路由下的回退信号）
   ...
   | ✅ 找到上游已修复版本 | 输出【上游依赖修复评估线索】，**进入步骤 3.6** 确认升级可行性（不直接跳步骤 4——「上游已修复」只是线索，仍需 3.6 评估升级风险/包管理器/验证链才能成为方案） |
...
3.6 **上游依赖修复评估**（可选，根因明确为上游依赖 bug 时触发——与 3.5 配对的「乐观分支」）
   ...
   > 💡 与 3.5 的区分：3.5 是「无可行解」的悲观分支（暂停等用户决定）；3.6 是「上游已修复」的乐观分支（升级依赖即可）。两者时机相同（根因明确后），方向相反。

4. **影响范围评估** - 受影响的模块/功能

**工具限制**：✅ Read/Grep/SemanticSearch；✅ WebSearch（步骤 2.5 已知问题快搜、步骤 3.5 行业通病评估、步骤 3.6 上游依赖修复评估、打点失效逃生出口 5.5 专用）；❌ Edit/Write；❌ Bash 执行改变实现逻辑的命令
...
**1.2 技术分析**：先报告存在性验证结论 → 0.5 调研路由判断（🟢/🔵/🟣）→ 再按路由侧重依次输出现象、定位、根因；若触发步骤3.5，在根因后输出行业通病评估结论；若触发步骤 3.6，输出上游依赖修复评估结论；继续输出影响范围。
...
> ⚠️ solve-workflow **不执行任何 git 合并操作**（7 阶段均为分析/审查/执行/验证/回顾，无合并步骤）。本提示是**建议性提醒，非强制门控**——不运行脚本、不阻断流程、不进能力探索表。
> 完整触发条件与提示文本见 [reference.md](reference.md)「阶段 7 合并前覆盖率提示（非门控）」。
