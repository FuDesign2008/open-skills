...
- 信息不足时，挑出当前关键未知（优先级：目的 → 约束 → 成功标准），向用户提问
- 优先用你所在 Agent 的原生结构化提问能力，无则用 prose；
- **得到回答后，基于该回答提出下一个**，如此多轮问清，直到信息充分。
...
各阶段进行中，若信息不足以保证输出质量，按此硬纪律**提关键问题**；得到回答后，基于回答决定下一步——信息已充分则继续推进，否则追问下一个未知。
