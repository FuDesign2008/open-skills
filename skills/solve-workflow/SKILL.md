...
> 八阶段 PDCA 流程，阶段 1、3、4、5 只读不写；阶段 2 仅允许分析辅助性临时改动（须登记并在进入阶段 3 前回滚）。默认手动模式，说「自动模式」或「自动分析」进入全流程自动执行。执行完成后进入阶段 7；若阶段 8 结论为「未达标需再改」，可回到「分析问题」、「探索方案」或「审查方案」开启下一轮循环。
...
> 启动时（进入阶段 1 之前）必须执行本检查，核对 frontmatter `dependencies` 声明的全部 12 个强依赖 skill。
...
| 2 分析问题 | ✅ Read/Grep；✅ WebSearch（调研路由/上游依赖修复评估/逃生出口专用）；⚠️ 受限 Edit/Write（分析辅助性临时改动，须登记+回滚，见阶段2正文） | 继续进入阶段3 | 存在性结论／根因／影响范围／临时改动回滚验证 |
...
| 🖥️ 浏览器操控 | 阶段2（分析问题）+ 阶段7（检查验证） | 浏览器可复现问题的运行时复现与修复验证；通道选择委托 `browser-debug-toolkit` |
| 🌐 Web 调研 | 阶段2（调研路由 / 打点逃生出口） | 经 `known-issue-research` 统一委托 `effective-web-research` |
| 💡 方案设计 | 阶段3（探索方案） | 辅助多方案生成与对比 |
| 📝 计划制定 | 阶段5（制定计划） | 辅助生成结构化执行计划 |
| ⚡ 代码执行 / 🧪 测试驱动 / 🔧 构建修复 | 阶段6（执行计划） | 批量编排 / 先写测试 / 构建错误修复 |
...
| 路径 | 适用场景 | 要求 |
|------|----------|------|
| 完整路径 | 新功能开发、多模块联动、需求模糊 | 阶段 1～8 全部执行；优先使用 `brainstorming` 类增强能力 |
...
**工具限制**：✅ Read/Grep/SemanticSearch；✅ WebSearch（调研路由与快搜、上游依赖修复评估、`runtime-evidence-debug` 的逃生出口（Phase 6）专用）；✅ Edit/Write 限上述分析辅助改动；✅ Bash 运行只读验证类命令——运行被调试应用/复现步骤仍需用户确认（见「打点调试」小节）
...
- `browser-debug-toolkit`：浏览器可复现问题优先用浏览器 DevTools 复现并实时检查运行时状态（DOM 树、计算样式、盒模型等），比代码层打点更高效；「问题现象描述」步骤已完成浏览器复现的，本小节仅在静态分析仍受阻时继续升级。
...
**阶段 2 分析问题**：先报告存在性验证结论 → 调研路由结论（🟢/🔵/🟣）→ 再按路由侧重依次输出现象、定位、根因；调研路由的快搜/通病评估结论按 `known-issue-research/reference.md` 模板输出；若触发「上游依赖修复评估」步骤，输出其评估结论；继续输出影响范围。**若本阶段做过临时改动，最后输出「临时改动清单 + 回滚验证」（见「临时改动权限与回滚门控」）。**
...
- AI 输出审查报告后，等用户响应：
  ...
  - 用户说「重选方案」→ 回到阶段3方案选择环节
- **循环上限**：无（用户自主控制）

### 审查报告输出字段

审查报告输出格式见 [reference.md](reference.md)「阶段 4 审查报告」。

> ⛔ **[手动模式 — 阶段4 出口]** 输出审查报告后**立即停止**，等用户明确说「通过」后进入阶段5；说「修改方案」则进入优化→重新审查循环。
> 🤖 **[自动模式]** AI 自动判定通过/不通过；通过进入阶段5，不通过自动优化后重新审查（上限3轮，超限暂停）。
...
审查通过后，输出结构化的**设计摘要**供阶段 5 和 6 引用：
...
此摘要不是独立文件，而是阶段 4 输出的一部分，供后续阶段引用。
...
> 原则：本阶段调用 `learn-and-improve` skill 执行复盘与经验沉淀（结构化复盘 + 沉淀价值判断 + 载体选择 + 有效性验证），**不内联重复其方法论**；本阶段仅保留 solve-workflow 特有的编排（循环决策、收尾/总结文档、覆盖率提示）。不默认写入文件；若用户明确要求写入规则、创建 skill 或根据改进点再改，则进入新一轮「制定计划 → 执行计划」；亦可回到「审查方案」重新选定或调整方案。
