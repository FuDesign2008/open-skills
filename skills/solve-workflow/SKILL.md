...
> **一次只问一个最关键的问题**——这是硬纪律，不靠自觉。信息不足时，挑出当前**最关键**的 1 个未知（优先级：目的 → 约束 → 成功标准），用「单问题 + 多选项」的 prose 格式提问（平台无关，见下方提问格式），得到回答后再问下一个。Claude Code 上可用 AskUserQuestion 工具将该格式工具化（天然一次一问）；Cursor / OpenCode 等无此工具时直接用 prose 格式即可。

**禁止**：一次抛多个问题 / 一次列多个疑问点让用户逐个回答——增加用户认知负担、降低答复质量。

**为什么**：多问题同抛 → 用户负担重、易漏答、AI 难聚焦；一次一问 → 渐进收敛、每个问题基于上一个回答精化。

各阶段进行中，若当前信息不足以保证输出质量，按此硬纪律每次只提出 **1 个最关键的问题**，待用户回答后再继续（或提下一个问题）。
