...
version: "2.5.3"
user-invocable: true
description: "open-skills 内部公共浏览器操控底座 — 通过 CDP Proxy 直连用户日常浏览器（Chrome / Edge / Chromium 系），天然携带登录态，提供 HTTP API（/new /eval /click /clickAt /setFiles /scroll /screenshot /navigate /close）用于动态页面、登录态访问、交互操作、媒体提取、视频截帧。被 effective-web-research 与 browser-debug-toolkit 强依赖（frontmatter dependencies），也可被用户直接调用。Triggers: 「浏览器操控」「CDP 操控」「登录态浏览器操控」「动态页面操控」「浏览器自动化」 / browser access, cdp control, login-state browser control, dynamic page control, browser automation. For debugging UI/CSS/DOM rendering issues, use browser-debug-toolkit instead; this skill is about *operating* the browser to reach content, not inspecting rendered layout."
---

# browser-access Skill

> **来源声明**：本 skill 的脚本与方法论移植自 [`eze-is/web-access`](https://github.com/eze-is/web-access) v2.5.3（MIT License, Copyright (c) 一泽 Eze），移植日期 2026-07-21。原项目持续迭代，本副本为分叉，必要时可从上游 rebase。MIT 许可证要求保留此版权与来源声明。

## 在 open-skills 中的定位

本 skill 是 open-skills 仓库内的**公共浏览器操控底座**：

- 承载 CDP Proxy 脚本（`scripts/`）+ 通用浏览器操控方法论（本文 + `references/`）；
- 被 `effective-web-research`（联网调研向）与 `browser-debug-toolkit`（调试向）通过 frontmatter `dependencies` 强依赖——两者各自只写「本职化用法」，浏览器操控的执行体统一委托给本 skill；
- 也可被用户直接调用，完成任何需要真实浏览器的联网/操控任务。
...
> `${CLAUDE_SKILL_DIR}` 由 agent 运行时自动注入为本 skill 的目录；在纯 shell 手动执行时，替换为本 skill 的绝对路径（如 `~/.claude/skills/browser-access` 或实际安装位置）。
...
检查通过后，必须在回复中向用户直接展示以下须知，再启动 CDP Proxy 执行操作：
...
> **与 effective-web-research 的分工**：来源可信度纪律（CRAAP + E-E-A-T、权威优先、时效、交叉验证）由 `effective-web-research` skill 承担；本 skill 负责"如何拿到内容"（操控），不重复"如何评估内容可信度"（纪律）。两者正交叠加。
...
- 必须在子 Agent prompt 中写 `必须加载 browser-access skill 并遵循指引`，子 Agent 会自动加载 skill，无需在 prompt 中复制 skill 内容或指定路径。
...
> 来源可信度评估（权威性、时效、交叉验证等纪律）交给 `effective-web-research` skill；本 skill 只负责"抵达一手来源"。
