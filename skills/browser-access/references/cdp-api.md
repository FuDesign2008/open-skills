...
- 启动：由 `${CLAUDE_SKILL_DIR}/scripts/check-deps.mjs` 自动启动并等待就绪（**不要**手动运行 cdp-proxy.mjs）
...
### POST /eval?target=ID
执行 JavaScript 表达式，POST body 为 JS 代码。返回 `{ "value": <结果> }`；若 JS 抛错或结果不可序列化，返回 `{ "error": "<message>" }`（DOM 节点不能直接返回，必须提取属性或用 `JSON.stringify` 包裹）。
```bash
# 单行表达式
curl -s -X POST "http://localhost:3456/eval?target=ID" -d 'document.title'

# 提取结构化数据（务必 JSON.stringify，否则 DOM 节点不可序列化 → error）
curl -s -X POST "http://localhost:3456/eval?target=ID" -d 'JSON.stringify({h: document.body.scrollHeight, title: document.title})'

# 多行 / 含引号的复杂脚本：用 --data-binary @file 读文件，避免 shell 拆解
echo '(() => { const els = document.querySelectorAll("*"); return JSON.stringify(els.length); })()' > /tmp/eval.js
curl -s -X POST "http://localhost:3456/eval?target=ID" --data-binary @/tmp/eval.js
```
