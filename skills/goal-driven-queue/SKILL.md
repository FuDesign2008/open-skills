...
version: "0.14.0"
user-invocable: true
description: "Queue orchestrator over child engines (goal-driven / solve / opsx-solve / jira-fix / opsx-jira-fix). Triggers:「goal 批量」「跑队列」「任务队列」「无人值守队列」「把需求加入队列」「空闲时段跑任务」「批量 triage」「确定度评估」「解法复用」/ goal-driven-queue, goal batch, run queue, unattended task queue, batch triage, certainty assessment, approach reuse. Each conversation auto-isolates its backlog under .goal-driven/queues/. Enqueue adds batch triage (equivalence merge, root-cause clustering, suggested kill before interview), a derived certainty/difficulty band as both ordering key and routing feed, and approach reuse; within a priority level cards order by information leverage. Serial consumption, caps, acceptance. Jira lists enqueue here (also via jira-fix-queue / opsx-jira-fix-queue; legacy jira-fix-batch / opsx-jira-fix-batch). Do NOT use for a single ad-hoc long run (goal-driven-workflow), a single Jira issue (jira-fix-workflow / opsx-jira-fix-workflow), or setting up cron/schedulers."
dependencies:
...
|-------|------------------|-------------|-----------------|
| 1 Enqueue & Pre-approve | ✅ Read; Write limited to `.goal-driven/queues/<queue-id>/` (cards, archived cards, triage record) | ⛔ stop before starting any run | validated task card(s) + triage record |
...
For each pending card in the order defined by [reference.md](reference.md) § Task Card (priority level → **information leverage** descending → certainty band → FIFO as the final tie-break), until caps hit or the queue drains:
