...
- ❌ 未发现 + 合并意图已确认（即上节触发条件命中）→ **不得静默跳过**：
  ...
  2. 写入留痕：`【覆盖率门控跳过】未检测到 test-coverage-analyzer skill，门控不可用。时间：<ISO 时间戳>。决策人：系统（环境缺漏）。`（留痕位置：`.jira-fix/{JIRA-ID}/08-merge.md` 和 PR 描述）
...
- **合并后发现**（无法回退）：如实报告漏跑，写入留痕：
  ...
  留痕位置：`.jira-fix/{JIRA-ID}/08-merge.md` 和 PR 描述（与显式跳过留痕位置一致）
