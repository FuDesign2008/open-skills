...
| 阶段5：执行计划 | 前置创建分支 + 执行 + Linter + 自动生成分析报告 | 展示分支计划确认后创建 + 执行 + Linter |
...
| 1 读取 Jira | 完成→⏸️ 确认进入阶段2 | 自动继续 | Jira 信息摘要 |
| 2 分析问题 | 存在性验证失败→停止；完成→⏸️ 确认进入阶段2.5 | 存在性验证失败→停止；完成→自动继续 | 根因／影响范围 |
| 2.5 难度分级 | 极难→⛔ A/B 选择；非极难→⏸️ 确认进入阶段3 | 极难→⛔ 终止；非极难→自动继续 | 难度等级 |
...
**[👤 手动]** 输出后附上：「⏸️ 前置检查完成。进入**阶段1：读取 Jira 信息**，回复「确认」继续。」
**[🤖 自动]** 自动进入阶段1，无需用户确认。
...
**[👤 手动]** 输出后附上：「⏸️ Jira 信息读取完成。进入**阶段2：分析问题**，回复「确认」继续。」
**[🤖 自动]** 自动进入阶段2，无需用户确认。
...
**[👤 手动]** 输出后附上：「⏸️ 问题分析完成。进入**阶段2.5：难度分级**，回复「确认」继续。」
**[🤖 自动]** 自动进入阶段2.5，无需用户确认。
...
**[👤 手动]** 非极难时，输出后附上：「⏸️ 难度分级完成（等级：X）。进入**阶段3：探索与审查方案**，回复「确认」继续。」
...
**[👤 手动]** 展示将要创建的分支列表（工程 / 基础分支 / 分支名），用户确认后执行；涉及多工程时逐一确认。

保存到 `.jira-fix/{JIRA-ID}/00-branch.md`，更新 state.json `branch` 字段。

**[👤 手动]** 分支创建后附上：「⏸️ 修复分支已创建，即将开始代码修改，回复「确认」继续。」
**[🤖 自动]** 自动继续执行计划。
...
> ⛔ **[手动模式 — 阶段7 出口]** 创建 PR 并展示 URL 后**立即停止**，在末尾**必须附上**：
