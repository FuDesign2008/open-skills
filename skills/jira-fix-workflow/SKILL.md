...
| 阶段8：合并 PR/MR 与收尾 | CI 通过后自动合并，清理分支，同步主分支 | 创建 PR/MR 后停止，用户 Approve 后 AI 合并并清理 |
...
| 8 合并 PR/MR 与收尾 | ⛔ 创建 PR/MR 后停，用户 Approve 后 AI 合并 | CI 通过后自动合并 | 合并状态／分支清理报告 |
...
**🔌 增强能力集成**：若发现「📋 代码审查」类能力（如 `requesting-code-review`、OMC `code-reviewer` agent），创建 PR/MR 后自动发起正式 Code Review 流程。

### 创建 PR/MR

使用平台 CLI 创建合并请求：**GitHub** 用 `gh pr create`；**GitLab** 用 `glab mr create`。标题格式与 commit message 保持一致：

```
fix(<scope>): <subject> [JIRA-ID]
```

PR/MR 描述必须包含：根因、修复方案、修改文件清单、验证场景（功能/边界/回归各 ≥2）、Jira 链接。

**GitLab（`glab`）**：合并用 `glab mr merge`（具体 flag 与团队默认 merge 方式一致，如是否 squash）。下文「同步主分支」中的 `main` 仅为示例，默认分支以远程为准（GitLab 常见 `main` 或 `master`）。

### [🤖 自动模式] 自动合并

1. 创建 PR/MR，等待 CI 检查通过
...
4. 切换并同步默认分支：`git checkout main && git pull origin main`（将 `main` 换成本仓库默认分支名）
5. 删除本地修复分支：`git branch -d fix/jira-fix-[JIRA-ID]`

### [👤 手动模式] 确认后合并

1. 创建 PR/MR，展示 PR/MR URL 和描述摘要，**立即停止，等用户完成 Code Review 并确认合并**
2. 用户确认（说「合并」「merge」「OK」等）后，AI 依次执行：
   - 合并：**GitHub** `gh pr merge --merge`；**GitLab** `glab mr merge`（参数与团队策略一致）
   - 删除远程修复分支
   - 切换并同步默认分支（`git checkout main && git pull origin main`，`main` 换成本仓库默认分支名）
   - 删除本地修复分支

> ⛔ **[手动模式 — 阶段8 出口]** 创建 PR/MR 并展示 URL 后**立即停止**，在末尾**必须附上**：
> 「⏸️ PR/MR 已创建，请完成 Code Review。是否合并并清理分支？回复「确认」继续。」
> 🤖 **[自动模式]** CI 通过后自动合并，执行分支清理与主分支同步，无需用户干预。

### 完成输出字段

PR/MR URL、合并状态、分支清理状态（本地/远程）、主分支同步状态。保存到 `.jira-fix/{JIRA-ID}/08-merge.md`，同时更新 state.json `current_phase: "completed"`。
...
批量修复不在本 skill 内部循环执行，而是将每个 Jira bug 作为独立任务，交由**任务编排工具**依次调用 `jira-fix [URL]`。OpenCode + oh-my-opencode 场景优先使用 `/ulw-loop`；其他平台按当前可用能力选择 `/ralph`、`/autopilot`、`/ultrawork` 等命令。
...
**示例（OpenCode + oh-my-opencode）**：

```
/ulw-loop 批量修复以下 Jira bug：
YNOTR-12167 YNOTR-12168 https://your-jira.example.com/browse/YNOTR-12169
```

其他平台可将 `/ulw-loop` 替换为当前可用的批量编排命令。执行过程中，编排工具应持续更新批量进度文档；执行结束后，输出进度文档路径与最终汇总。
