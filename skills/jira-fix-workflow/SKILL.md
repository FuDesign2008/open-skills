...
> 「⏸️ 阶段5（执行计划）完成，请审查代码。是否进入**阶段6：提交 PR**？回复「确认」继续，或告知需要调整。」
...
> ⛔ **[手动模式 — 阶段6 出口]** 展示提交计划和验证场景后**立即停止**，在末尾**必须附上**：
...
> ⛔ **[手动模式 — 阶段7 出口]** 创建 PR/MR 并展示 URL 后**立即停止**，在末尾**必须附上**：
> 「⏸️ PR/MR 已创建，请完成 Code Review。是否合并并清理分支？回复「确认」继续。」
> 🤖 **[自动模式]** CI 通过后自动合并，执行分支清理与主分支同步，无需用户干预。

### 完成输出字段

PR/MR URL、合并状态、分支清理状态（本地/远程）、主分支同步状态。保存到 `.jira-fix/{JIRA-ID}/07-merge.md`，同时更新 state.json `current_phase: "completed"`。
...
2. 对每个 issue 依次调用 `jira-fix [URL]`
3. 单个 issue 失败不阻断后续 issue
4. 跟踪整体进度，处理完成后输出批量汇总
...
|------|------|
| Issue | Jira ID 或 URL |
| 状态 | 待处理 / 处理中 / 已完成 / 审查未通过（超限） / 失败 / 已跳过 |
