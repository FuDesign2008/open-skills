...
> **`--retry` 场景首选策略**：静态分析已做过一次，再次静态分析大概率触达同样的边界。应优先考虑打点调试，用运行时事实重新锚定根因。
...
| 5. 日志分析 | 分析实际调用链和数据流，给出根因结论，更新 `02-analysis.md` |
...
2. **结果处理**：找到 → 更新根因假设，回到步骤 5；未找到 → 输出「已排除假设清单」，**暂停等用户决定**（继续打点 / 外部支持 / 带假设进入方案阶段）
