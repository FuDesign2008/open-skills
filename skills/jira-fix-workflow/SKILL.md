...
- **⚠️ 主动提问（硬纪律：一次一问、多轮问清）**：信息不足时**每轮只提 1 个最关键的问题**（优先级：目的 → 约束 → 成功标准），**得到回答后基于该回答提下一个——多个未知分多轮问清**。即：一条消息不堆叠多个（**不得一次抛多个**，违反者见各阶段 Red Flags）；**每轮基于上一轮回答精化**。用你所在 Agent 的原生结构化提问能力（无则用下方 prose 格式）：
