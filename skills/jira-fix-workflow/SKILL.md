...
| 阶段4：制定计划 | ❌ | ❌ | 只输出计划文本 |
| 阶段5：执行计划 | ✅ | ✅ | 前置 git 创建分支；运行构建/lint/tsc 前须按 `node-version-discipline` 对齐 `.nvmrc` Node 版本，再进行代码修改 |
...
0. **Node 版本对齐（前置）**：运行 Linter/TypeScript/构建命令前，先调用 `node-version-discipline` skill 按 `.nvmrc`（兜底 `engines.node`）对齐 Node 版本。所有 `npm run`/`npx` 命令须在单条调用内 `source ~/.nvm/nvm.sh && nvm use <版本> && <命令>`，切换后 `node -v` 确认。
...
4. **副作用检查**：确认 Node 版本已对齐 `.nvmrc`（见 `node-version-discipline`）、Linter 和 TypeScript 检查均通过（补充确认阶段5结果），检查改动是否在其他模块引发新问题
...
- 副作用检查：Node(.nvmrc vX) ✅/⚠️ 未对齐，Linter ✅/❌，TS ✅/❌，功能副作用 ✅/❌
- 逻辑完整性：...
