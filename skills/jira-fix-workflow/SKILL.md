...
| PDCA | 阶段 | 说明 |
|------|------|------|
| **Plan** | 阶段 1～4 | 读取 Jira → 分析问题（含难度分级）→ 探索与审查方案 → 制定计划 |
| **Do** | 阶段 5 | 执行计划 |
| **Check** | 阶段 6 | 检查验证 |
| **Act** | 阶段 7～8 | 提交 PR/MR → Review 与合并 |

验证未达标时，按失败类型返回对应阶段（阶段 5/3/2）开启新一轮循环。
...
| 阶段 | 核心目标 | 预期落点 |
|------|---------|---------|
| 0 前置检查 | 环境与数据就绪 | 检查通过摘要 |
| 1 读取 Jira | 获取完整 Bug 上下文 | Jira 信息摘要 |
| 1.5 理解对齐 | 对齐问题理解（手动模式） | 问题复述＋歧义点 |
| 2 分析问题 | 验证存在性＋定位根因 | 存在性结论＋根因＋影响范围 |
| 2.5 难度分级 | 风险评估与路径决策 | 难度等级＋行为决策 |
| 3 探索与审查方案 | 多方案对比＋深度审查 | 方案对比表＋审查报告＋设计摘要 |
| 4 制定计划 | 可执行修改清单 | 文件修改清单＋执行顺序 |
| 5 执行计划 | 按计划实现 | 执行报告＋测试结论 |
| 6 检查验证 | 独立验证实现质量 | 验证结果（诚实标注执行/待执行） |
| 7 提交 PR/MR | 代码提交＋Jira 回写 | PR URL＋Jira 状态 |
| 8 Review 与合并 | 合并＋分支清理 | 合并状态＋清理报告 |
...
| 路径 | 适用等级 | 要求 |
|------|---------|------|
| 精简路径 | 🟢 容易 | 阶段 3 方案可只输出 1 个方案 + 风险说明；阶段 4 计划可合并到方案描述中。阶段 6 和 7 不可跳过 |
...
- **已知问题快搜触发条件满足，却以「先看代码」「先打点」为由跳过 WebSearch**（平台静默失败类问题应先搜再分析）
...
| 容易问题走完整 8 阶段流程 | 流程过重，启动成本高 | 🟢 容易可使用精简路径（方案+计划合并），不跳过阶段 6/7 |
| 验证报告将「设计了场景」写成「验证已通过」 | 用户接受虚假通过结论 | 每项必须标注「已执行（命令+输出）」或「待执行（操作指引）」 |
| 审查通过后无设计摘要 | 执行阶段缺少设计决策依据 | 阶段 3 通过后输出 Goals/Non-Goals/Decisions/Risks 摘要 |
| 执行完任务后不即时更新 checkbox/TODO | 进度不透明，中断恢复时状态不准 | 每完成一项立即更新状态，不得批量更新 |
| 方案中夹带非必要功能 | 方案臃肿，执行成本膨胀 | 剔除非必要功能与过度设计（YAGNI） |
| 业务逻辑变更无测试覆盖且未提醒 | 回归风险无人知晓 | 执行报告中检查测试覆盖，无覆盖时明确提醒 |
| 复杂 Jira issue 未拆解直接分析 | 多系统纠缠导致方案不聚焦 | 阶段 1.5 先拆解独立模块、依赖、处理顺序 |

---

## 输出详细度控制（自适应）

- 🟢 容易（精简路径）：精简输出，方案可 1 个 + 风险说明，计划可合并
- 🟡 中等（标准路径）：标准输出，按各阶段模板
- 🟠 困难 / 🔴 极难（完整路径）：详细输出 + 代码示例 + 完整审查记录
