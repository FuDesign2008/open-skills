...
| 🌐 Web 调研 | research, web, look up, investigate, web-research, effective-web-research | 阶段2（已知快搜/行业通病/上游依赖修复/打点逃生） | 外部 web 调研纪律：Step 0 分流 + 4 口诀 + 严格模式（见 `effective-web-research` skill） |
...
| 📋 代码审查 | code-review, review, requesting-review | 阶段7 | 代码审查（注：`solution-review`、`code-design-review`、`hybrid-debug`、`runtime-evidence-debug`、`browser-debug-toolkit` 均已通过 frontmatter `dependencies` 强绑定，不走环境探索） |
...
**触发条件**（满足任一即执行）：
- 根因明确为上游依赖 bug（具名第三方库/框架的已知缺陷）
- 已知问题快搜找到上游已修复版本（搜到根因后顺手查 Changelog/Release Notes）

**执行**：加载 `upstream-dependency-debug` skill，按其方法论执行——4 步决策顺序（归因上游？→ 查 Changelog/Release Notes → 低 semver 风险升级优先 → 仅在升级不可行时 workaround）+ 通用升级工程纪律（包管理器一致性、升级后验证链、semver 风险、去重检查）+ 结果表。

| 结论 | 处理方式 |
|------|---------|
| ✅ 上游已修复，升级低风险 | 进阶段3方案探索，**推荐升级为首选方案** |
| ⚠️ 升级有风险（major/breaking） | 阶段3方案探索中升级与 workaround 并列，由风险取舍决定 |
| ❌ 上游未修复 / 升级不可行 | 回退 workaround，但**注明临时性**及追踪的上游 issue |
