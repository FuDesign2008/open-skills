...
2. 用户确认（说「合并」「merge」「确认」「OK」等）后，AI 依次执行：
   ...
   **前置检测**：环境探索是否发现 `test-coverage-analyzer` skill？
   - ❌ 未发现 → 静默跳过本门控，直接进入步骤 2.2 合并（不报错、不阻断、不留痕）
   - ✅ 发现 → 执行下方门控步骤

   **门控步骤**（本步骤拥有独立 Bash 权限，运行 test-coverage-analyzer 脚本，不计入阶段 8「仅平台 CLI 合并命令」约束）：
   ...
   3. 按判定矩阵处理：
      ...
      | 执行结果 | 🤖 自动模式 | 👤 手动模式 |
      |---|---|---|
      | ✅ 报告生成 + 覆盖率达标 | 继续进入步骤 2.2 合并 | 提示通过，等用户再次确认合并 |
   ...
   4. 显式跳过（仅手动模式，用户主动选择跳过门控）：必须在 `.jira-fix/{JIRA-ID}/08-merge.md` 和 PR 描述写入留痕：
