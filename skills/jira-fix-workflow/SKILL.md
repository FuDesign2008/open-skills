...
- **⚠️ 主动提问（硬纪律：一次一问、多轮问清）**：信息不足时**每轮只提 1 个最关键的问题**（优先级：目的 → 约束 → 成功标准），**得到回答后基于该回答提下一个——多个未知分多轮问清**。即：一条消息不堆叠多个（**不得一次抛多个**，违反者见各阶段 Red Flags）；该问几个分几轮问清，**且每轮基于上一轮回答精化**（渐进收敛），**不得误解为「全程只能问一个」**而带着信息缺口继续。用你所在 Agent 的原生结构化提问能力（无则用下方 prose 格式）：
