...
- **⚠️ 主动提问（硬纪律：一次只问一个）**：信息不足时**只提 1 个最关键的问题**（优先级：目的 → 约束 → 成功标准），得到回答后再提下一个——**不得一次抛多个**（违反者见各阶段 Red Flags）。格式平台无关（Claude Code 可用 AskUserQuestion，Cursor/OpenCode 用下方 prose）：
