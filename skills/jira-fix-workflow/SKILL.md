...
**调试优先级规则（路径差异类 bug）**：当现象表现为"A 路径正常、B 路径异常"且两条路径汇入同一下游时（如拖拽 vs 粘贴、移动端 vs 桌面端、旧版 vs 新版），**优先动作**：
1. 定位两条路径的**共用下游打点**（现成的 console.log、日志埋点）
2. 请用户分别在 A/B 路径下复现并贴出同一打点的日志
3. 对比日志中**对象的实际结构、类型、关键字段值**
4. 仅当日志对比无法定位时，才回到代码推理

**效果**：10 倍于凭代码推理的效率，尤其对"符号遮蔽""运行时类型不一致""race"类隐性 bug。

**反例**：YNOTR-13467 第二轮 bug（符号遮蔽），阶段 1.2 初版浪费 20 分钟做 7 个假设推理，最终是用户贴出 `upload-directive.service.ts:97` 打点日志（`upload - attachment - size() - file: File {isBrains: false, id: "...", size: undefined}`）1 秒锁定根因。
...
当本次修改**涉及以下任一领域**时，阶段 6 必须额外委托 Oracle 做 skeptical review（不可仅靠 Linter/TS/Jest 下结论）：
...
| **符号/类型遮蔽** | `import { File } from '业务模型'` 遮蔽 DOM 原生构造器、项目类名与全局 API 冲突、类型签名与运行时类型不一致 |

**审查清单必含项**：Oracle prompt 中必须显式要求核查「运行时对象形状」——即"调用 `new X(...)` 得到的对象字段是否与期望的 DOM 原生类型一致"。静态类型检查无法触及此盲区（TS 会用 `import` 路径覆盖全局类型），**必须通过日志或 DevTools 观察实际对象结构**。

**调用方式**：

```
task(subagent_type="oracle", load_skills=[], run_in_background=true, prompt="""
Skeptically review YNOTR-XXXXX fix. Look for:
...
**判定规则**：
- Oracle 报告含 🔴 / P0 / blocking 级别问题 → 本阶段判定 ❌ 未达标，按失败类型返回阶段 5（实现缺陷）或阶段 3（方案缺陷）
...
> ⛔ **[手动模式 — 阶段8 出口]** 展示 PR/MR URL 后**立即停止**，在末尾**必须附上**：
