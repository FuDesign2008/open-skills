...
version: "1.0.0"
user-invocable: true
description: 当版本迭代开始时需要创建 release 分支时使用，涵盖基础分支确认、命名规范分析、远程创建（避免 tracking 错误）、本地同步、分支保护、验证等场景。适用于 GitLab、GitHub 等平台，支持单仓库和多仓库。触发词：创建release分支、开release分支、开分支、迭代分支、create release branch。
---

# Git 仓库 Release 分支创建工作流

> 与 `git-release-workflow` 配对使用：本 skill 覆盖迭代开始时的分支创建，`git-release-workflow` 覆盖迭代结束时的发版操作。
> 核心原则：**先远程创建，再本地同步**——避免本地分支 tracking 指向错误的基础分支。
...
| 仓库 | 版本号 | 基础分支 | release 分支名（待确认） |
|------|--------|---------|----------------------|
| repo-A | 8.2.70 | master | release/8.2.70 |
...
> ⚠️ release 分支命名可能不是简单的 `release/X.Y.Z`，需要查历史。
...
# 2. Tracking 指向正确
git branch -vv | grep '^\*'
# 期望：包含 [origin/release/X.Y.Z]

# 3. release-branch 文件正确（如有）
cat release-branch
# 期望：release/X.Y.Z
