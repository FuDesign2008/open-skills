...
version: "1.0.0"
user-invocable: true
description: "去除中文文本中的 AI 生成痕迹，改写得更自然。适用于编辑审阅、人性化改写。触发词：去AI痕迹、人性化改写、去痕、humanizer-zh。勿用于英文为主文本（用 humanizer）、技术评审文档（tech-review-doc）、公众号改写（article-writer）。"
---

# Humanizer-zh: 去除 AI 写作痕迹

> **上游：** 适配自 [op7418/Humanizer-zh](https://github.com/op7418/humanizer-zh)（MIT）；核心译自 [blader/humanizer](https://github.com/blader/humanizer)；实用清单参考 hardikpandya/stop-slop；模式来源维基百科 Signs of AI writing。见 `LICENSE`。
>
> 澄清提问遵循意图「一次一问」；由运行环境的 Agent 选用原生提问能力，不硬编码平台专属工具。
