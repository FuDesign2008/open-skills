...
2. **Runnable UI** — Resolve through the measurement channel ladder (preferred first): a component-workbench story from the living artifact’s Stories subsection (or a detected Storybook-style workbench) → a lightweight isolated harness (stub out host/engine dependencies) → the in-product preview route → a full app/browser launch (heaviest, last resort).
...
6. **Report** — Write the Verify section of the living artifact (or the sibling report named in Spec source). Template: [reference.md](reference.md). Include coverage (measured vs inventory), the **channel declaration** (`Channel: storybook | isolated-harness | preview | full-app`), and, when the project declares or shows a host/platform floor (an engine older than the workbench browser), a note that the real host/device remains the final authority — workbench PASS is not platform-floor proof. Component-test results may be cited as supporting evidence. For multi-theme scope, report per theme. That Verify section **is** the measured report hosts require. Overall is **PASS** only when every critical visual row in scope is PASS or `accepted-residual`. Overall is **not PASS** if any critical row is unmeasured, `MISSING-style`, unaccepted `DRIFT`, `HARDCODED` when the spec required a token, the spec itself is incomplete, or the running UI shows an **invented solid fill** against Spec transparent/`none`, a **fragment-cut** graphic against a single Assets export, a **generic wrapper** that does not match instance geometry, a **self-invented text-color alias** when Spec recorded a Figma variable, or a visible graphic still on a **remote design URL**.
...
- Reporting a verdict without declaring the measurement channel used.
- Treating a component-workbench PASS as proof against the project’s host/platform floor — the real host remains the final authority.
