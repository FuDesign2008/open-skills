...
批量 OPSX Jira 修复不在本 skill 内部循环执行，而是由任务编排工具将每个 Jira issue 拆成独立 OPSX 修复任务，逐个调用 `opsx-jira-fix [URL]`。与普通 `jira-fix-workflow` 不同，批量关系判断必须沉淀到 OpenSpec artifacts，不能只保留在临时进度文档或对话上下文中。
...
4. 对每个 issue 依次调用 `opsx-jira-fix [URL]`，不得绕过阶段 0～7 的 OpenSpec 记录、验证和归档要求。
