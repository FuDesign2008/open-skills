...
5.5. **打点失效升级**：若日志分析后根因置信度仍为「模糊/未知」（如日志未捕获关键路径、打点位置偏差、运行环境差异），不再重复打点，升级为 WebSearch 或查阅框架文档寻找已知问题模式和排查思路。升级后仍无法定位时，在手动模式下暂停与用户讨论；自动模式标注「根因未完全确认」并继续后续阶段（降低置信度标记）。
