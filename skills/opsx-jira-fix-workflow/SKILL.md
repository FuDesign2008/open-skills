...
6. 检查 OpenSpec 命令：优先使用 `openspec list`、`openspec status`、`openspec validate`；不可用时可直接读写 `openspec/`，但必须说明降级。
...
| 阶段 | 工具权限 | 停止点 | 必须输出 |
|------|---------|--------|---------|
| 0 前置检查 | Read/Grep/Bash（只读检查） | 任一检查失败 | 模式、能力表、change 定位结果 |
| 1 读取 Jira | Jira API/jira-read（只读） | Jira 描述不足时问 1 个问题 | Jira 数据写入 design.md 的 Jira Context |
| 2 分析问题 | Read/Grep（只读） | 根因模糊→打点调试→等用户日志 | design.md 的 Problem Analysis / Root Cause / Impact |
| 3 创建 Change | OPSX 原生 skills/Write | 手动模式：等用户确认 change 名称 | proposal.md、specs/、design.md、tasks.md |
| 4 探索方案 | Read/Grep（只读） | 手动模式：等用户选择方案；审查不通过→重审 | design.md 的 Options / Decisions |
| 5 制定计划 | Read/Write（仅 tasks.md） | 手动模式/困难/极难：暂停确认 | tasks.md 完整任务清单 |
| 6 执行验证 | 全部 | 验证失败→不得提交 PR | 代码修改、checkbox 更新、验证报告 |
| 7 提交收尾 | Git/Jira/OpenSpec tools | archive 失败→停止 | PR、Jira 评论+状态、archive 结果 |

### 阶段工具约束表

| 阶段 | ✅ 允许 | ❌ 禁止 |
|------|---------|---------|
| 0 前置检查 | Read、Grep、Bash（只读检查）、Jira API（只读） | Edit、Write、Git 写操作 |
...
| 阶段 | 核心目标 | OpenSpec 落点 |
|------|---------|--------------|
| 0 前置检查 | 确认环境就绪，定位 change | — |
| 1 读取 Jira | 获取最新 Jira 数据并写入 artifacts | design.md → Jira Context |
| 2 分析问题 | 定位根因、评估影响和难度 | design.md → Problem Analysis / Root Cause / Impact |
| 3 创建 Change | 生成完整的 OpenSpec artifacts | proposal.md、specs/、design.md、tasks.md |
| 4 探索方案 | 2-3 个方案、审查覆盖、选定最优 | design.md → Options / Decisions |
| 5 制定计划 | 可执行的任务清单 | tasks.md（checkbox 格式） |
| 6 执行验证 | 修复代码、确保测试、验证闭环 | tasks.md checkbox、design.md → Verification Notes |
| 7 提交收尾 | PR、Jira 回写、archive、沉淀 | archive → openspec/specs/、openspec/changes/archive/ |
...
| 路径 | 输出详细度 | 适用场景 |
|------|-----------|---------|
| 精简路径 | 精简输出：结论 + 关键证据，省略完整推理过程 | 容易问题（≤3 文件，根因清晰） |
| 增量路径 | 标准输出：结论 + 证据 + 简要推理 | 中等问题（4-10 文件） |
| 完整路径 | 详细输出：完整分析、推理链、所有对比项 | 困难/极难问题 |

### PDCA 对应表

| PDCA 阶段 | 对应工作流阶段 | 说明 |
|-----------|--------------|------|
| **Plan** | 阶段 0-5 | 前置检查→读取→分析→创建 Change→探索方案→制定计划 |
| **Do** | 阶段 6.1-6.3 | 创建分支→执行任务→Superpowers 增强 |
| **Check** | 阶段 6.4 | 验证（OpenSpec 校验 + 工程验证 + 行为对照 + Jira 对照 + 副作用检查） |
| **Act** | 阶段 7 | 提交 PR→Jira 回写→Archive→分支收尾→AI 沉淀 |
...
> **重试场景首选策略**：静态分析已做过一次，再次静态分析大概率触达同样的边界。应优先考虑打点调试，用运行时事实重新锚定根因。
...
| 5. 日志分析 | 分析实际调用链和数据流，给出根因结论，更新 `design.md` 的 Problem Analysis 小节 |
| 6. 清理建议 | （可选）根因确认后，说明哪些打点可删除，哪些值得保留为正式监控 |

**逃生出口 5.5**（步骤 5 后根因仍为「模糊/未知」时触发）：

> 打点只能观测「代码层执行路径」，无法穿透平台层/宿主环境的静默拦截。升级为 WebSearch 或查阅框架文档寻找已知问题模式和排查思路。升级后仍无法定位时，手动模式暂停与用户讨论；自动模式标注「根因未完全确认」并继续后续阶段（降低置信度标记）。
...
> 🚩 **Red Flags（阶段 4）**：
> - ❌ 方案夹带非必要功能，超出根因修复范围（违反 YAGNI）
> - ❌ 审查只覆盖根因，未检查 spec 覆盖和副作用
> - ❌ 审查通过后 design.md 未更新审查结论中的取舍和风险
> - ❌ 只有 1 个方案就跳过对比和审查
...
> 🚩 **Red Flags（阶段 6）**：
> - ❌ 完成任务后未立即更新 checkbox（延后批量更新）
> - ❌ 验证报告将「设计了场景」写成「验证已通过」
> - ❌ 单元测试失败仍进入 6.4 验证
...
| 批量勾选 tasks.md checkbox | 阶段 6.4 验证器报 CRITICAL 虚假未完成 | 每个任务完成后立即用 StrReplace 更新对应 checkbox |
| 验证报告将"设计了场景"写成"验证已通过" | 用户接受虚假的通过结论 | 每项必须标注"已执行（命令+输出摘要）"或"待执行（操作指引）" |
| delta spec requirement 不包含 SHALL/MUST | `openspec validate` 报错 | requirement 描述必须含 SHALL 或 MUST；使用 `#### Scenario:` 添加场景 |
| 各阶段跳过工具限制约束 | 只读阶段误写代码，破坏阶段隔离 | 每个阶段必须遵守阶段工具约束表 |
| 方案中夹带非必要功能 | OpenSpec 变更范围膨胀，增加审查和回归风险 | 剔除非必要功能与过度设计（YAGNI），额外改动必须标注「额外优化」 |
| 打点调试多轮后仍不升级 | 陷入打点死循环，浪费时间 | 日志分析后置信度仍为「模糊/未知」时，触发 5.5 逃生出口转向 WebSearch |
| 审查通过后 design.md 未更新审查结论 | 执行阶段缺少设计决策依据 | 审查通过后核对 design.md 的 Goals/Decisions/Risks 是否完整 |
| 使用 Superpowers 凭记忆调用 | 规则过期导致错误行为 | 调用增强能力前必须先读取其当前 SKILL.md |
