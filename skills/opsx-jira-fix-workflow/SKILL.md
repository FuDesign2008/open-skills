...
> - ❌ 用户直接说「merge MR / 合并 MR」但 AI 跳过门控直接合并（v2 新增场景）——门控触发锚点是「合并动作」本身，不依赖分支收尾决策；用户直接下达合并指令时也必须先跑门控
> - ❌ AI 即将合并但未运行门控（隐式漏跑）却继续合并或合并后未留痕（v2 新增）——隐式漏跑视为门控未通过，合并前发现须暂停补跑；合并后发现无法回退，必须如实报告并写入留痕
> - ❌ 未装 test-coverage-analyzer 的项目合并时静默跳过且不留痕（v2 收紧）——前置检测未发现 skill 时必须输出提示并留痕，由用户决定是否继续，不得静默放行
