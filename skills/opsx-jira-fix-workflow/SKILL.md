...
**Triggers** (any one triggers; takes priority over entering Phase 3):
...
- The current attempt is a retry (`--retry` / "继续修复") — a fix based on static analysis has already been tried, but the problem persists.

> **Retry-Scenario Preferred Strategy** (variant-specific): Static analysis has already been performed once; repeating it will likely hit the same boundary. Prefer instrumentation debugging to re-anchor the root cause with runtime facts.
...
| 5. Log analysis | Analyze the actual call chain and data flow, produce a root-cause conclusion, update the Problem Analysis section of `design.md` |
...
2. **Result handling**: Found → update the root-cause hypothesis, return to step 5; Not found → output "list of ruled-out hypotheses", **manual mode: pause and discuss with the user; auto mode: mark "root cause not fully confirmed" and continue to subsequent phases with a reduced-confidence tag**.

**Tool restrictions** (variant-specific): ✅ Read/Grep to help determine probe locations; ❌ Edit/Write (probe code is added manually by the user, or by the AI only after explicit user confirmation); ❌ must not run reproduction steps without user confirmation.
<!-- /SYNC-SECTION: instrumentation-debugging -->
