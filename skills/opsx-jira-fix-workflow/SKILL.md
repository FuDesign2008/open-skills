...
- **继续修复**：触发词含“继续修复”“再次修复”“从上次继续”或 `--retry` 时，从现有 OpenSpec change、`tasks.md` checkbox 和当前 Git 分支恢复上下文。
...
6. 继续修复时，使用 `openspec status --change <name>`、`openspec show <change-name>`、`tasks.md` checkbox 和当前 Git 分支恢复进度。
...
读取最新 Jira 数据，并在阶段 3 创建 OpenSpec change 时摘录到 `proposal.md` 或 `design.md` 的 Jira Context 小节：
...
输出进入 `openspec/changes/<change-name>/design.md` 的 Problem Analysis / Root Cause / Impact 小节。若阶段 2 尚未创建 change，先在对话中输出分析结论，阶段 3 创建后必须补写到 `design.md`。
