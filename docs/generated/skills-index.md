...
> **请勿手改。** 源数据：`skills/<name>/SKILL.md`。生成时间：2026-05-19。
> 
...
| **git-release-finish** | 1.0.0 | 是 | 当 Git 仓库需要发版时使用，涵盖 tag 命名规范不统一、主分支名称各异（master/main/develop）、release 分支合入主干存在冲突、MR/PR 分支混入无关文件等场景。适用于 GitLab、GitHub、Gitea 等平台，支持单仓库和多仓库。触发词：发版、打tag、发布版本、版本发布、release流程、release工作流、git-release、multi-repo release。 |
...
| **jira-fix-analyze** | 1.0.0 | 否 | 由 jira-fix-workflow 阶段2调用。执行存在性验证、代码定位、已知问题快搜、根因分析、行业通病评估与打点调试，输出分析结论供阶段2.5使用。禁止独立调用。 |
...
| **jira-fix-execute** | 1.0.0 | 否 | 由 jira-fix-workflow 阶段5和阶段6调用。执行代码修改、Linter/TS 检查与验证确认（含自动回退路径）。禁止独立调用。 |
| **jira-fix-plan** | 1.0.0 | 否 | 由 jira-fix-workflow 阶段3和阶段4调用。执行方案探索、方案审查循环（含自动/手动模式）与计划制定。禁止独立调用。 |
| **jira-fix-submit** | 1.0.0 | 否 | 由 jira-fix-workflow 阶段7和阶段8调用。执行代码提交、PR/MR 创建、Jira 状态回写（含 jira_add_comment 陷阱规避）与合并清理。禁止独立调用。 |
| **jira-fix-workflow** | 4.0.0 | 是 | 当用户说「修复这个 bug [URL]」「帮我修复 [URL]」「jira-fix [URL]」「自动修复 [URL]」「强制修复 [URL]」「继续修复」「从上次继续」时触发。适用于从 Jira 链接出发、对单个 bug 进行端到端修复的场景。 |
...
| **solve-analyze** | 1.0.0 | 否 | 由 solve-workflow 阶段1.2调用。执行存在性验证、代码定位、已知问题快搜、根因分析、行业通病评估与打点调试。禁止独立调用。 |
| **solve-finish** | 1.0.0 | 否 | 由 solve-workflow 阶段5、6、7调用。执行计划执行、检查验证与回顾总结（含 AI 工程沉淀建议）。禁止独立调用。 |
| **solve-plan** | 1.0.0 | 否 | 由 solve-workflow 阶段3调用。执行方案深度审查（含自动/手动循环审查机制）。禁止独立调用。 |
| **solve-workflow** | 2.0.0 | 是 | 当用户说"明确问题"、"分析问题"、"探索方案"、"审查方案"、"制定计划"、"执行计划"、"检查验证"、"回顾总结"，或"继续分析"、"深入分析"、"修改方案"、"完善方案"、"优化方案"、"更新计划"、"修订计划"、"修改计划"，或"自动模式"、"自动分析"、"自动解决"时触发。适用于 bug 修复、代码重构、功能开发等需系统性分析的复杂任务。 |
