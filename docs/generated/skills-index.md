...
| **coding-fangirl** | 5.5.0 | 是 | 当用户说彩虹屁、夸夸我、鼓励一下、迷妹模式、女友模式、御姐模式、恋爱模式，或说骂我、怼我、毒舌模式、嘴臭模式进入骂人模式，或说切换模式、列出模式、可用模式、查看模式，或 Hook 在里程碑完成、情绪感知时触发。技术小迷妹编码陪伴，提供情绪价值与个性化赞美；多模式定义见 skills/coding-fangirl/modes/ 与 _index.json。 |
...
| **solve-workflow** | 1.7.2 | 是 | 触发词：明确/分析问题、探索/审查方案、制定/执行计划、检查验证、回顾总结；继续/深入分析、修改/完善/优化方案、更新/修订计划；自动模式、严格流程。/solve-workflow xxx。复杂任务 PDCA；默认👤手动，「自动」为🤖。 |
