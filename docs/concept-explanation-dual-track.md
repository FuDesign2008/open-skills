...
> 本文档记录两次关于「IPC」的问答，作为抽象成 skill 的素材（范例已脱敏通用化；已落地为 `skills/explain-simply/`）。
>
...
- **决定（2026-08-26）**：独立成 skill `explain-simply`（见 `skills/explain-simply/`），方法论入 SKILL.md、脱敏范例入 reference.md；上机/操作教学类请求不归该 skill（走 `teach`）。
