...
1. 七阶段问题解决工作流（含「审查方案」）（从“明确问题”到“回顾总结”）  
2. 性能分析到优化验证的闭环流程  
...
3. 命令验证：
   ...
   - `/encourage`
4. 如果安装命令不可用或未检索到：
   - 可暂用 **通用安装**：`npx skills add FuDesign2008/open-skills -g`（仅 SKILL.md，无 Hooks/Commands），或查看 [docs/INSTALL.md](INSTALL.md) 与 Issues；待上架后再用 `/plugin-add` 做 **全能力安装**。

## 4) README 对外口径（已同步）

`README.md` 已区分 **全能力安装** 与 **通用安装**：

- **全能力安装**（已上架）：`/plugin-add open-skills`
- **通用安装**（未上架 / 未检索到）：`npx skills …`，或等 Marketplace 可用后再做全能力安装

避免用户误以为「未上架也能按 slug 直接装全量插件」。
